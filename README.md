...
Two evaluation tracks, six model types, conformal uncertainty quantification, cost-sensitive maintenance policy, MLflow registry, XGBoost native serving, Evidently monitoring, Docker Compose.
...
|-------|-----|------|--------------------|
| XGBoost deployment regressor (24 raw features) | 68.7 | 81.0 | 101.6 |
| XGBoost research regressor (276 features, RUL cap 125) | 24.5 | 21.0 | — |

The research regressor uses the full 276-feature pipeline (rolling mean/std/slope + lag features) and clips predictions at 125 cycles — matching standard CMAPSS evaluation convention. The deployment regressor uses only 24 raw sensor readings for single-cycle API inference.

### Uncertainty Quantification (FD001 test set)

| Method | Coverage | Interval Width |
|--------|----------|----------------|
| Conformal risk score (XGBoost, α=0.10) | ≥ 90% | calibrated per `conformal_qhat.json` |
| Quantile XGBoost RUL (q=0.10/0.90) | ~80% | varies by cycle depth |
| MC-Dropout LSTM (T=30 forward passes) | empirical | σ reported per prediction |
| Bootstrap C-index CI (CoxPH, n=200) | 95% | reported with lower/upper bounds |

All interval endpoints are exposed via the `/predict` API response (`risk_lower`, `risk_upper`, `rul_lower`, `rul_upper`).
...
### Cost-Sensitive Policy — Expected Maintenance Cost (FD001, per engine)

The `CostPolicy` maps each prediction to an expected cost based on action probabilities and configurable replacement/inspection/missed-failure costs (defaults: replace=5000, inspect=500, failure=50000). The `/predict` endpoint returns `action` (`replace` / `inspect` / `continue`) and `expected_cost` per unit.

### Results Overview

A 9-panel summary figure is generated by `scripts/plot_results.py` and saved to `reports/results_overview.png`. It covers: model comparison bar chart, RUL scatter with quantile error bars, residual distribution, PR curves, calibration diagram, confusion matrix, cost-vs-threshold curve, RUL uncertainty bands, and RMSE comparison.
...
│   ├── deep/lstm.py         # PyTorch LSTM + SlidingWindowDataset + MC-Dropout
│   ├── deep/tcn.py          # Temporal Convolutional Network (causal dilated convolutions)
│   └── survival/cox.py      # CoxPHFitter with simulated censoring + bootstrap C-index CI
├── evaluation/
│   ├── splits.py            # track_a_split() + track_b_split()
│   └── metrics.py           # PR-AUC, F1, MAE, C-index, Brier, ECE, business_metrics
├── policy/
│   └── notification.py      # ServiceAlert + evaluate_alert + CostPolicy (expected cost per action)
├── mlops/
│   └── tracking.py          # MLflow run logging
├── service/
│   ├── api.py               # FastAPI: /health, /predict, /explain, /schedule, /batch_score
│   ├── schemas.py           # Pydantic v2 request/response (+ uncertainty intervals + action + cost)
...
│   ├── robustness.py        # inject_missing, inject_noise, inject_flatline (sensor corruption)
...
│   ├── plot_results.py      # 9-panel results figure → reports/results_overview.png
│   ├── demo_results.py      # per-engine inference demo with uncertainty bounds
│   └── check_rmse.py        # RMSE evaluation against RUL_FD001.txt ground truth
├── tests/
...
│   └── test_robustness.py   # 19 tests: missing/noise/flatline sensor corruption
├── docker/
│   ├── Dockerfile.api
│   ├── Dockerfile.mlflow
...
Conformal prediction gives coverage guarantees without distributional assumptions. A single calibration pass over a held-out set produces a `qhat` threshold that yields ≥ 90% coverage on any future exchange-able sample. Unlike Bayesian credible intervals, it requires no model modification and works on any black-box predictor.

**Why CostPolicy over fixed-threshold alerting?**  
A fixed risk threshold treats false negatives and false positives symmetrically. In practice, missing a failure (unplanned downtime + emergency repair) costs an order of magnitude more than an unnecessary inspection. `CostPolicy` maps each prediction to an expected cost: `E[cost] = p_replace × c_replace + p_inspect × c_inspect + p_fail × c_fail`. The action with the lowest expected cost is returned, allowing operators to tune costs to their fleet economics.
...
`risk_lower`/`risk_upper` are 90% conformal prediction intervals on the risk score. `rul_lower`/`rul_upper` are quantile XGBoost bounds (q=0.10/0.90). `action` and `expected_cost` come from `CostPolicy` (configurable cost parameters in `config.yaml`).
